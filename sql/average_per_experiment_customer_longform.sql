...
-- Re-formatted in long form for plotting purposes
...
(
  SELECT 
    customer_id,
    average_med AS avg_purchase,
    purchase_rank,
    FALSE AS customer_added_non_med_to_order
  FROM 
    ranked_averages
)
UNION ALL 
(
  SELECT 
    customer_id,
    average_nonmed AS avg_purchase,
    purchase_rank,
    TRUE AS customer_added_non_med_to_order
  FROM 
    ranked_averages
)
